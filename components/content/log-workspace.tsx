...
type LogWorkspaceListItem = {
  title: string;
  slug: string;
};

type LogWorkspaceLog = {
  title: string;
  summary: string;
  slug: string;
  date: string;
...
type LogWorkspaceProps = {
  logs: LogWorkspaceListItem[];
  selectedLog?: LogWorkspaceLog;
};

function getLogHref(log: LogWorkspaceListItem, index: number) {
  return index === 0 ? "/logs" : `/logs/${log.slug}`;
}
...
export function LogWorkspace({
  ...
  if (!selectedLog) {
    return (
      <div className="log-page mx-auto w-full max-w-[1760px] px-2 pt-10 sm:px-4 sm:pt-14 xl:px-5">
        <div className="surface-card rounded-[1.8rem] p-6 sm:p-8">
          <p className="text-sm text-[var(--muted)]">当前还没有可展示的日志内容。</p>
        </div>
      </div>
    );
  }

  const hasHeadings = selectedLog.headings.length > 0;

  return (
    <div className="log-page mx-auto grid w-full max-w-[1760px] gap-6 px-2 pt-6 pb-6 sm:gap-8 sm:px-4 sm:pt-8 xl:px-5">
      <section
            ...
            <details open className="group">
              ...
              <nav className="mt-4">
                <ul className="grid gap-1.5 border-l border-[var(--border)] pl-3">
                  {logs.map((log, index) => {
                    const active = log.slug === selectedLog.slug;

                    return (
                      <li key={log.slug}>
                        <Link
                          href={getLogHref(log, index)}
          ...
          <article className="rounded-[2rem] bg-[linear-gradient(180deg,rgba(255,251,237,0.88)_0%,rgba(255,255,255,0.68)_100%)] p-6 shadow-[0_18px_50px_rgba(212,190,133,0.12)] sm:p-8 lg:p-10 dark:bg-[linear-gradient(180deg,rgba(56,47,34,0.24)_0%,rgba(31,35,45,0.22)_100%)] dark:shadow-[0_18px_50px_rgba(0,0,0,0.18)]">
            <div className="grid gap-6">
              <header className="grid gap-4 border-b border-[var(--border)] pb-6">
                <div className="flex flex-wrap items-center gap-2 text-xs tracking-[0.16em] text-[var(--muted)]">
                  <span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-1 text-[color:var(--accent)]">
                    {selectedLog.period ?? "日志"}
                  </span>
                  <span>{formatDate(selectedLog.date)}</span>
                  {selectedLog.updated ? (
                    <span>更新于 {formatDate(selectedLog.updated)}</span>
                  ) : null}
                  {selectedLog.readingTime ? (
                    <span>{selectedLog.readingTime} 分钟阅读</span>
                  ) : null}
                </div>
              ...
              {hasHeadings ? (
                <details className="mobile-toc surface-card rounded-[1.4rem] p-4 lg:hidden">
                  ...
                  <ul className="mt-4 grid list-none gap-3 text-sm">
                    {selectedLog.headings.map((heading) => (
                      <li
                        key={heading.id}
                        className={`leading-6 ${
              ...
              <div className="content-body">
                <MdxContent code={selectedLog.body} />
              </div>
            </div>
          </article>
        </div>
